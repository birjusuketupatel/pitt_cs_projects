...
public class SimBank{
	...
	public SimBank(int num_tells, boolean one_queue, double hrs, double cust_per_hour, double min_per_transaction, int max_capacity, long seed) {
	...
	public void runSimulation() {
		...
		while(event_list.size() > 0) {
			...
			double next_serve = R.exponential(serve_rate); //generates length of next service
			double next_arr = R.exponential(arr_rate_min); //generates time till next arrival
			...
			if(curr_event instanceof ArrivalEvent && curr_time < sim_time) {
				...
				//creates a new customer, sets his length of service, arrival time, and id
				Customer new_cust = new Customer(cust_id, curr_time);
				new_cust.setServiceT(next_serve);
				cust_id ++;
				
				//generates next ArrivalEvent and adds it to the future event list
				next_arr_time = curr_time + next_arr;
				ArrivalEvent next_arrival = new ArrivalEvent(next_arr_time);
				event_list.offer(next_arrival);
				
				//places customer in queue if bank is under capacity
				//sends customer to cust_list if bank is over capacity
				if(curr_capacity < tot_capacity) {
					...
					//if queue is empty and teller is not busy (customer does not wait), resets queue ID to -1
			...
			for(int i = 0; i < tell_list.size(); i ++) {
				...
				if(curr_teller.isBusy() == false && curr_queue.peek() != null) {
					...
					double len_service = next_cust.getServiceT();
					double finish_time = curr_time + len_service;
	...
	public void showResults() {
		...
		System.out.println("ID   ARRIVAL   SERVICE");
	...
	private void printCustomer(Customer cust) {
		...
		if(this.wasServed(cust)) {
			...
			System.out.printf("%-4d %-10.2f %-8.2f\n", cust.getId(), cust.getArrivalT(), cust.getServiceT());
		}
	...
	//customer was not served if getWaitT throws an ArithmeticException
