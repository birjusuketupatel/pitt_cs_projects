...
/*root is RArgTypeOp*/              {
                                      ...
                                      $<tptr>$ = MakeTree(RArgTypeOp, NullExp(), argtype);/* lchild is unknown*/
                                    }
                                 IDENTIFIER_rec
                                    {
                                      SetLeftChild($<tptr>2, $3);
                                      if ( NodeKind(LeftChild(RightChild($<tptr>2))) == DUMMYNode )/* no comma */
                                         SetRightChild( $<tptr>2, NullExp() );
                                      else
					{
					  tree t = $<tptr>2; /* head of the tree */
                                      ...
                                      $$ = $<tptr>2;  /* return to FormalParameterList_rec */
                                    }
                                  /* right child is to be put in by IDENTIFIER_rec  */
                              |  VALnum INTnum
/*root is VArgTypeOp*/              {
                                      argtype = MakeTree(VArgTypeOp, NullExp(), NullExp());
                                      $<tptr>$ = MakeTree(VArgTypeOp, NullExp(), argtype);/* lchild is unknown*/
				    }
                                 IDENTIFIER_rec
                                    {
                                      SetLeftChild($<tptr>3, $4);
                                      if ( NodeKind(LeftChild(RightChild($<tptr>3))) == DUMMYNode )/* no comma */
                                         SetRightChild( $<tptr>3, NullExp() );
                                      else
					{
					  tree t = $<tptr>3; /* head of the tree */
                                          /* delete the tangling right most child */
                                          while ( t->RightC != argtype )
                                                 t = t->RightC;
                                          t->RightC = NullExp();
                                      ...
                                      $$ = $<tptr>3;  /* return to FormalParameterList_rec */
			            }
                                  /* right child is to be put in by IDENTIFIER_rec */
                              ;
